...
3. **文献引用**：
   - 只有第一章绪论允许返回文献引用
   - 第二章到第六章的 citations 必须返回空数组 []
   - 不能编造文献标题、作者或年份
   - 第一章正文中可以使用 GB/T 7714 风格描述或 [1] 这类编号提示
   - **第一章的 citations 数组只能填写“文献标题 / 成果名称 / 证据卡片标题”，不能填写完整参考文献串**
...
特别注意：JSON 返回中的 citations 只能填写标题级名称，不要填写完整参考文献串。"""
        if chapter_key == "chapter_1_introduction":
            user_message += "\n\n当前是第一章：允许在 citations 中返回文献标题或成果/证据名称。"
        else:
            user_message += "\n\n当前不是第一章：正文可以围绕项目设计、成果和证据展开，但 citations 必须返回空数组 []。"
